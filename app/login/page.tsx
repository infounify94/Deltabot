      ...
      <nav className="w-full flex justify-between items-center px-4 sm:px-6 py-4 glass-header z-50">
        <Link href="/" className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-emerald-500/20 flex items-center justify-center border border-emerald-500/30">
            <Activity className="text-emerald-400 w-5 h-5" strokeWidth={2.5} />
          </div>
          <div className="flex items-center">
            <span className="font-bold text-lg tracking-tight text-[var(--ink)]">Profit</span>
            <span className="font-bold text-lg tracking-tight text-emerald-400">Pilot</span>
          </div>
        </Link>

        <div className="flex items-center gap-2.5">
          <Link href="/" className="text-sm font-medium text-[var(--grey)] hover:text-[var(--ink)] flex items-center gap-1.5 transition-colors">
      ...
      <div className="flex-1 flex items-center justify-center p-4 sm:p-6 my-6 z-10">
        <div className="max-w-md w-full">
          <GlassCard className="p-6 sm:p-8 space-y-6 relative overflow-hidden">
            ...
            {/* Accent top line */}
            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-emerald-500 via-purple-500 to-teal-500" />

            {/* Mode Switcher Tabs */}
            <div className="grid grid-cols-2 bg-[var(--paper2)] p-1 rounded-lg border border-[var(--hair)] text-sm font-medium">
              <button
                type="button"
                onClick={() => {
                  setIsSignUp(false);
                ...
                className={`py-2 rounded-md transition-all ${!isSignUp ? 'bg-emerald-700 text-white shadow-subtle' : 'text-[var(--grey)] hover:text-[var(--ink)]'}`}
              >
                Sign In
              </button>
              <button
            ...
            <div className="relative flex items-center justify-center">
              ...
              <div className="relative bg-[#0b0c10] px-4">
                <span className="text-xs text-[var(--grey)] font-medium uppercase tracking-wider">
                  Or with email
                </span>
              </div>
            </div>
            ...
            <form onSubmit={handleEmailAuth} className="space-y-4 text-sm">
              {isSignUp && (
                <>
                  <div>
                    <label className="block font-medium text-[var(--grey)] mb-1.5 flex items-center gap-1.5">
                    ...
                    <input
                      ...
                      className="w-full bg-[var(--paper2)] border border-[var(--hair)] rounded-xl px-4 py-2.5 text-[var(--ink)] placeholder:text-[var(--grey)] focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full font-mono bg-[var(--paper2)] border border-[var(--hair)] rounded-xl px-4 py-2.5 text-[var(--ink)] placeholder:text-[var(--grey)] focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition"
                    />
                  </div>
                </>
              )}
              ...
              <div>
                <label className="block font-medium text-[var(--grey)] mb-1.5 flex items-center gap-1.5">
                  <Lock className="w-4 h-4 text-emerald-400" /> Password {isSignUp && <span className="text-xs text-[var(--grey)] font-normal">(min 6 chars)</span>}
                </label>
                <input
                  type="password"
                  required
              ...
              {isSignUp && (
                <div>
                  <label className="block font-medium text-[var(--grey)] mb-1.5 flex items-center gap-1.5">
                    <KeyRound className="w-4 h-4 text-emerald-400" /> Confirm Password
                  </label>
                  <input
                    type="password"
                    required
                    ...
                    className={`w-full bg-[var(--paper2)] border rounded-xl px-4 py-2.5 text-[var(--ink)] placeholder:text-[var(--grey)] focus:outline-none focus:ring-1 transition ${confirmPassword && password !== confirmPassword ? 'border-rose-500 focus:ring-rose-500' : 'border-[var(--hair)] focus:border-emerald-500 focus:ring-emerald-500'}`}
                  />
              ...
              <div className="pt-2">
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-emerald-700 hover:bg-emerald-800 text-white font-semibold py-3 px-4 rounded-xl shadow-[0_0_20px_rgba(79,70,229,0.3)] transition disabled:opacity-50 text-sm flex justify-center"
                >
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#050505] text-[var(--ink)] flex items-center justify-center text-sm font-medium">Loading...</div>}>
      <LoginContent />
    </Suspense>
  );
}
